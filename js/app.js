...
(function ($) {
	...
	function getKotoba() {
		const $kotoba = $('#kotoba');
		if($kotoba.length === 0) return;
		const url = ROOT + 'js/kotoba.json';
		$.get(url).done(res => {
			if (res.data.length > 0) {
				const index = Math.floor(Math.random() * res.data.length);
				$kotoba
                    .css('color','transparent')
                    .text(res.data[index].body)
                if(res.data[index].cite) $kotoba.append('<cite> ——  %s</cite>'.replace('%s',res.data[index].cite));
                $kotoba.css('color','white')
			}
		})
	}
